...
final class ConditionalOpsSpec extends AnyFreeSpec with Matchers {
  ...
  "ConditionalOps" - {
    ...
    "when: applies step only when predicate is true" in {
      ...
      val out1 = ConditionalOps.when(j => (j \ "n").asOpt[Int].exists(_ > 1))(step)(in).toOption.get
      out1 mustBe Json.obj("n" -> 2, "flag" -> true)

      val out2 = ConditionalOps.when(j => (j \ "n").asOpt[Int].exists(_ > 10))(step)(in).toOption.get
      out2 mustBe in
    }
    ...
    "ifExists: runs step only when path resolves to a single value" in {
      ...
      val out1 = ConditionalOps.ifExists(__ \ "old")(step)(in1).toOption.get
      out1 mustBe Json.parse("""{ "new": "x", "other": 1 }""")

      val in2  = Json.parse("""{ "other": 1 }""").as[JsObject]
      val out2 = ConditionalOps.ifExists(__ \ "old")(step)(in2).toOption.get
      out2 mustBe in2
    }
    ...
    "ifExists: path ambiguous (multiple matches) is treated as not existing → no-op" in {
      ...
      val out = ConditionalOps.ifExists(__ \ "arr" \ "b")(step)(in).toOption.get
      out mustBe in
    }
    ...
    "ifMissing: runs step when path is absent" in {
      ...
      val out = ConditionalOps.ifMissing(__ \ "ctx" \ "version")(step)(in).toOption.get
    ...
    "ifMissing: ambiguous (multiple matches) counts as missing → step runs" in {
      ...
      val out = ConditionalOps.ifMissing(__ \ "arr" \ "b")(step)(in).toOption.get
