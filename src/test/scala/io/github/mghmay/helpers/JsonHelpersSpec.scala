  ...
  "JsonHelpers" - {
    ...
    "movePath" - {
      ...
      "moves a scalar value; aggressively prunes source by default" in {
        ...
        val out = movePath(__ \ "a" \ "b", __ \ "x", in.as[JsObject]).toOption.getOrElse(
      ...
      "gentle mode leaves a tombstone at the exact 'from' path (scalar)" in {
        val in  = Json.parse("""{ "a": { "b": 1 }, "x": null }""")
        val out =
          movePath(__ \ "a" \ "b", __ \ "x", in.as[JsObject],
      ...
      "creates destination parents as needed" in {
        ...
        val out = movePath(__ \ "a" \ "b", __ \ "dest" \ "inner", in.as[JsObject]).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "maintains keys at destination after write" in {
        ...
        val out = movePath(__ \ "a" \ "b", __ \ "x", in).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "overlap: to is inside from (capture then cleanup then set) is well-defined" in {
        ...
        val out = movePath(__ \ "a", __ \ "a" \ "b" \ "d", in.as[JsObject]).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "overlap: 'from' is inside 'to' (cleanup first prevents duplication)" in {
        ...
        val out = movePath(__ \ "x" \ "a" \ "b", __ \ "x", in.as[JsObject]).toOption.getOrElse(
          fail("Expected successful transformation"))
        out mustBe Json.parse("""{ "x": 1 }""")
      }
      ...
      "no-op when from == to" in {
        val in  = Json.parse("""{ "a": { "b": 1 } }""")
        val out = movePath(__ \ "a" \ "b", __ \ "a" \ "b", in.as[JsObject]).toOption.getOrElse(
          fail("Expected successful transformation"))
        out mustBe in
      }
      ...
      "fails with JsError when source path does not exist" in {
        ...
        val left            = movePath(__ \ "missing", __ \ "x", in.as[JsObject]).swap.getOrElse(
          fail("Expected successful transformation"))
      ...
      "fails with JsError when source path resolves to multiple values (ambiguous)" in {
        ...
        val left            = movePath(__ \ "arr" \ "b", __ \ "x", in.as[JsObject]).swap.getOrElse(
          fail("Expected successful transformation"))
        val (badPath, msgs) = left.errors.head
      ...
      "fails when destination path uses array index (unsupported in setPath)" in {
        ...
        val left = movePath(__ \ "a" \ "b", __ \ "x" \ 0, in.as[JsObject]).swap.getOrElse(
          fail("Expected successful transformation"))
    ...
    "copyPath" - {
      ...
      "copy keeps source intact and writes destination" in {
        ...
        val out = copyPath(__ \ "a" \ "b", __ \ "x", in).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "copy creates destination parents as needed" in {
        val in  = Json.parse("""{ "a": { "b": 1 } }""").as[JsObject]
        val out = copyPath(__ \ "a" \ "b", __ \ "dest" \ "inner", in).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "copy works when to is a descendant of from" in {
        val in  = Json.parse("""{ "a": { "b": 1 } }""").as[JsObject]
        val out = copyPath(__ \ "a", __ \ "a" \ "copy", in).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "copy preserves source even when destination overlaps with source structure" in {
        ...
        val out = copyPath(__ \ "data", __ \ "data" \ "backup", in).toOption.getOrElse(
          fail("Expected successful transformation"))
    ...
    "mapAt" - {
      ...
      "can change type (string -> object) via validator" in {
        ...
        val out = mapAt(__ \ "user" \ "name", in)(splitName).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "propagates mapping failure and prefixes error paths with the target path" in {
        ...
        val left            = mapAt(__ \ "a" \ "name", in)(vf).swap.getOrElse(fail("Expected successful transformation"))
        val (badPath, msgs) = left.errors.head
      ...
      "fails with JsError when path does not exist (or not unique)" in {
        ...
        val left            = mapAt(__ \ "missing", in)(v => JsSuccess(v)).swap.getOrElse(
          fail("Expected successful transformation"))
        val (badPath, msgs) = left.errors.head
        badPath mustBe (__ \ "missing")
      ...
      "fails with JsError when path resolves to multiple values (ambiguous)" in {
        ...
        val left            = mapAt(__ \ "arr" \ "b", in)(v => JsSuccess(v)).swap.getOrElse(
          fail("Expected successful transformation"))
        val (badPath, msgs) = left.errors.head
        badPath mustBe (__ \ "arr" \ "b")
      ...
      "supports mapping to JsNull (keeps the key with a null value)" in {
        ...
        val out = mapAt(__ \ "a" \ "b", in)(_ => JsSuccess(JsNull)).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "fails when attempting to set into an array segment (unsupported in setNestedPath)" in {
        ...
        }.swap.getOrElse(fail("Expected successful transformation"))
    ...
    "aggressivePrunePath" - {
      ...
      "removes a nested key and prunes empty parents" in {
        ...
        val out = aggressivePrunePath(__ \ "a" \ "b", in.as[JsObject]).toOption.getOrElse(
          fail("Expected successful transformation"))
        out mustBe Json.parse("""{ }""")
      }
      ...
      "fails when path does not exist" in {
        val in   = Json.parse("""{ "a": { "b": "value" } }""")
        val left = aggressivePrunePath(__ \ "a" \ "missing", in.as[JsObject]).swap.getOrElse(
          fail("Expected successful transformation"))
      ...
      "fails when a parent exists but is not an object" in {
        val in   = Json.parse("""{ "a": 1 }""")
        val left = aggressivePrunePath(__ \ "a" \ "b", in.as[JsObject]).swap.getOrElse(
          fail("Expected successful transformation"))
      ...
      "fails when path starts with an array segment" in {
        val in   = Json.parse("""{ "a": [ { "b": 1 } ] }""")
        val left = aggressivePrunePath(__ \ 0, in.as[JsObject]).swap.getOrElse(
          fail("Expected successful transformation"))
      ...
      "fails with JsError when given an empty JsPath" in {
        val in   = Json.parse("""{ "a": { "b": 1 } }""")
        val left = aggressivePrunePath(JsPath(Nil), in.as[JsObject]).swap.getOrElse(
          fail("Expected successful transformation"))
    ...
    "gentlePrunePath" - {
      ...
      "removes only the node and keeps now-empty parents" in {
        val in  = Json.parse("""{ "a": { "b": { } } }""")
        val out = gentlePrunePath(__ \ "a" \ "b", in.as[JsObject]).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "fails when path does not exist or unsupported" in {
        val in   = Json.parse("""{ "a": { } }""")
        val left = gentlePrunePath(__ \ "a" \ "missing", in.as[JsObject]).swap.getOrElse(
          fail("Expected successful transformation"))
      ...
      "fails when pruning an array index" in {
        val in   = Json.parse("""{ "a": [1, 2, 3] }""").as[JsObject]
        val left =
          gentlePrunePath(__ \ "a" \ 0, in).swap.getOrElse(fail("Expected successful transformation"))
    ...
    "deepMergeAt" - {
      ...
      "deep merges into an existing object and creates parents as needed" in {
        ...
        val out =
          deepMergeAt(in.as[JsObject], __ \ "ctx",
            Json.obj("version" -> 3, "env" -> "prod")).toOption.getOrElse(
            fail("Expected successful transformation"))
      ...
      "creates parents when merging at a missing path" in {
        val in  = Json.parse("""{ }""")
        val out =
          deepMergeAt(in.as[JsObject], __ \ "meta" \ "build", Json.obj("id" -> 42)).toOption.getOrElse(
            fail("Expected successful transformation"))
      ...
      "fails with JsError when the path contains array segments (IdxPathNode)" in {
        val in                  = Json.parse("""{ "a": [ { "b": 1 } ] }""")
        val left                = deepMergeAt(in.as[JsObject], __ \ "a" \ 0 \ "b", Json.obj("x" -> 2)).swap.getOrElse(
          fail("Expected successful transformation"))
        val (badPath, messages) = left.errors.head
        badPath mustBe (__ \ "a" \ 0 \ "b")
        messages.head.message must include("arrays not supported")
      }

      "deepMergeAt replaces non-object parent with object" in {
        val in  = Json.obj("a" -> 1)
        val out =
          deepMergeAt(in.as[JsObject], __ \ "a" \ "b", Json.obj("x" -> 1)).toOption.getOrElse(fail("..."))
        out mustBe Json.obj("a" -> Json.obj("b" -> Json.obj("x" -> 1)))
      }
    }
    ...
    "setNestedPath" - {
      ...
      "sets a value at a nested path, creating parents" in {
        val in  = Json.parse("""{ }""")
        val out = setNestedPath(__ \ "a" \ "b" \ "c", JsString("v"), in.as[JsObject]).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "overwrites an existing value" in {
        ...
        val out = setNestedPath(__ \ "a" \ "b" \ "c", JsString("new"), in.as[JsObject]).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "sets an empty object at a leaf (does not remove the key)" in {
        ...
        val out = setNestedPath(__ \ "a", Json.obj(), in).toOption.getOrElse(
          fail("Expected successful transformation"))
      ...
      "sets an empty object at a deep path (keeps the leaf key)" in {
        ...
        val out = setNestedPath(__ \ "a" \ "b" \ "c", Json.obj(), in).toOption.getOrElse(
          fail("Expected successful transformation"))
        out mustBe Json.parse("""{ "a": { "b": { "c": { } } } }""")
      }

      "setNestedPath replaces non-object parent with object" in {
        val in  = Json.obj("a" -> 1)
        val out = setNestedPath(__ \ "a" \ "b", JsNumber(2), in.as[JsObject]).toOption.getOrElse(fail("..."))
        out mustBe Json.obj("a" -> Json.obj("b" -> 2))
      }
    }
  }
}
